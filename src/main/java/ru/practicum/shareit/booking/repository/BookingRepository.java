...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findByUserIdAndCurrentState(Long userId, LocalDateTime now);
    ...
    @Query("select book " +
           ...
           "and (?2 between book.start and book.end) " +
           "order by book.start desc")
    List<Booking> findByOwnerIdAndCurrentState(Long userId, LocalDateTime now);

    List<Booking> findByBookerIdAndStartAfter(Long userId, LocalDateTime now, Sort sort);

    List<Booking> findByItemUserIdAndStartAfter(Long userId, LocalDateTime now, Sort sort);

    List<Booking> findByBookerIdAndStatus(Long userId, Status status, Sort sort);

    List<Booking> findByItemUserIdAndStatus(Long userId, Status status, Sort sort);
    ...
    List<Booking> findByItemUserIdAndItemIdAndStatusAndStartBefore(Long userId,
                                                                   Long itemId,
                                                                   Status status,
                                                                   LocalDateTime now,
                                                                   Pageable page);
